...
public class CircularBitmapDrawable extends Drawable {
    ...
    public CircularBitmapDrawable(final Bitmap bitmap) {
    ...
    public CircularBitmapDrawable(final int color) {
        paint = new Paint();
        paint.setAntiAlias(true);
    ...
    @Override protected void onBoundsChange(final Rect bounds) {
        ...
        path.reset();
        path.addCircle(viewRect.centerX(), viewRect.centerY(), Math.min(viewRect.width(), viewRect.height()) / 2, Path.Direction.CW);
    }
